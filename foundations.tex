...
\begin{theorem}{Quasiisomorphisms localize in the homotopy category}{Quis form a localizing class}
    ...
    Quasiisomosphisms form a localizing class in the homotopy category of an abelian category $K(\calA)$. We denote the derived category by
    \begin{equation*}
...
The three objects $A^\bullet, B^\bullet, \mathrm{cone}(f)$ fit into a long exact sequence of cohomology groups. On the level of the homotopy, we would like to put the boundary map on the same footing as the other ones, and form what is called a \emph{distinguished triangle}: \[\begin{tikzcd}
    & B^\bullet \arrow[rd] &                                       \\
...
\begin{proof}
    One needs to show that the two morphisms described below are homotopy inverses, which can be done by finding explicit chain homotopies.
\end{proof}
...
The morphisms between two chain complexes in $\mathrm{Ch}(\calA)$ tend to be quite big: in fact, they fit into a Hom-chain complex themselves:
...
\begin{proof}
    We only show this for $A^\bullet$ a single object concentrated in degree zero. Take an injective resolution $B^\bullet \to I^\bullet$. We see that
\begin{equation*}
...
\end{remark}
We mention briefly the Hochschild cohomology $HH^\bullet(X):=\Ext^\bullet(\O_{\Delta_X}, \O_{\Delta_X})$, and the following important theorem, which boils down to the local-to-global spectral sequence degenerating:
...
One can use the LES on hypercohomology and reduce to the case of vector bundles (this method of proof is called devissage), by using truncations.
...
\begin{definition}{Calabi-Yau category}{Calabi-Yau category}
    A category is called a Calabi-Yau $n$ category if it has a Serre functor given by just shifting by $n$.
\end{definition}
...
For example, $\O,\dots,\O(n)$ on projective space. Cannot have more, since then will have the cohomology of the canonical bundle $\omega=\O(-n-1)$ which is nonzero. In fact, Beilinson's theorem (to be proved below) shows that these form a full exceptional collection.
...
\todo{Need to define Mukai vector etc. Maybe don't need the action on cohomology however}
