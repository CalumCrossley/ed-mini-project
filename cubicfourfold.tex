In this section, we will review the deep link between K3 surfaces and cubic fourfolds. 
 ...
 A theorem of Hasset describes the so called \emph{special cubic fourfolds}, which are cubic fourfolds containing a surface $T$ which is not a complete intersection. They form a countable union of divisors $\calC_d$ in the moduli space $\P H^0(\O(3))/\P GL(6)$ of cubic fourfolds which are expected to be rational (the intuition is that for a cubic fourfold to be rational, one has to blow up a surface at some point). They also have associated K3 surfaces: Hassett shows that the primitive cohomology of a K3 surface embeds as the complement $\langle h^2, T\rangle^\perp$. 
...
The Hodge decomposition tells us that $$H^2(S,\mathbb{C})=H^{2,0}\oplus H^{1,1}\oplus H^{0,2}$$But both $h^{2,0}=h^{0,2}=1$ and hence $h^{1,1}=20$.  The Hodge diamond is thus: 
\begin{figure}[H]
\[\begin{tikzcd}[row sep=tiny, column sep=tiny]
  ...
  1 &   & 20 &   & 1 \\
    ...
    &   & 1  &   &  
  \end{tikzcd}\]
  \caption{Hodge diamond of K3 surface}
\end{figure}
Moreover, the intersection lattice is $H^2(X,\mathbb{Z})=\Lambda=(-E_8)^{\oplus 2}\oplus U^{\oplus 3}$ and $H^2(S,\mathbb{Z})=(1,20,1)$ has signature $(3,19)$.
...
\begin{tikzcd}[row sep=tiny, column sep=tiny]
  ...
  \end{tikzcd}
\end{center}
\caption{Hodge diamond of cubic fourfold}
\end{figure}
...
The left mutation with respect to the admissible subcategory spanned by the line bundles is, by \ref{prop:Mutations of semiorthogonal decompositions} $$\mL_{\langle \O, \dots, \O(n+1-d) \rangle}\simeq \mL_\O \circ \dots \circ \mL_{\O(n+1-d)}$$
